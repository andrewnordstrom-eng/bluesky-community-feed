...
interface WeightBarProps {
  ...
  /** If true, fill uses tongue-pink (penalty signal) — overrides `color`. */
  negative?: boolean
  /** Optional fill color (e.g. a signal color). Falls back to ginger primary. */
  color?: string
...
export function WeightBar({ label, value, negative = false, color, size = "md", className }: WeightBarProps) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className ?? ""}`}>
      ...
      <div className={`w-full ${barHeight} rounded-full bg-biscuit overflow-hidden`}>
        <div
          className={`${barHeight} rounded-full transition-all duration-500 ${negative ? "bg-tongue" : tinted ? "" : "bg-primary"}`}
          style={{ width: `${pct}%`, backgroundColor: tinted ? color : undefined }}
          role="progressbar"
          aria-valuenow={pct}
          aria-valuemin={0}
          aria-valuemax={100}
