              ...
              properties: {
                ...
                jetstream: {
                  ...
                  properties: {
                    ...
                    cursor_us: { type: 'string', description: 'Latest safely completed Jetstream cursor in microseconds' },
                    cursor_lag_ms: { type: 'number', description: 'Lag between the processed cursor and wall-clock time in ms' },
                    active_events: { type: 'number', description: 'Jetstream events currently being processed' },
                    pending_events: { type: 'number', description: 'Jetstream events waiting for a processing slot' },
                    inbound_paused: { type: 'boolean', description: 'Whether socket reads are paused for backpressure' },
                    pause_count: { type: 'number', description: 'Number of inbound backpressure pauses since process start' },
                    resume_count: { type: 'number', description: 'Number of inbound backpressure resumes since process start' },
                    overload_reconnect_count: { type: 'number', description: 'Reconnects triggered by the hard queue limit since process start' },
                    flow_control_failure_reconnect_count: { type: 'number', description: 'Reconnects triggered by socket pause/resume failures since process start' },
                    total_dropped_events: { type: 'number', description: 'Incoming events rejected at the hard queue limit since process start' },
                    failed_cursor_persistence_floor_us: { type: 'string', description: 'Fail-closed cursor floor retained after a dead-letter persistence failure' },
                    error: { type: 'string' },
                  },
